angular.module('todoApp', [])
  .controller('TodoListController', ['$scope', '$http', '$templateCache', function ($scope, $http, $templateCache) {
    ...
    $scope.groupDate = () =>{
      ...
      angular.forEach($scope.todos.data, function (data) {
        ...
        if (dateSlice >= $scope.todos.dateFrom && dateSlice <= $scope.todos.dateTo) {
          if (!object[dateSlice]) {
            object[dateSlice] = { date: dateSlice, sumsalemount: 0, sumcostamount: 0, gross: 0 };
    ...
    $scope.coppyDate = ()=>{
      ...
      var i, j;
      for (i = 0; i < $scope.todos.setDateMonth.length; i++) {
          for (j = 0; j < $scope.todos.dataFilterChart.length; j++) {
              if ($scope.todos.setDateMonth[i].date == $scope.todos.dataFilterChart[j].date) {
                $scope.todos.setDateMonth[i].date = $scope.todos.dataFilterChart[j].date;
                $scope.todos.setDateMonth[i].sumsalemount = $scope.todos.dataFilterChart[j].sumsalemount;
                $scope.todos.setDateMonth[i].sumcostamount= $scope.todos.dataFilterChart[j].sumcostamount;
                $scope.todos.setDateMonth[i].gross= $scope.todos.dataFilterChart[j].gross;
               }
          }
      }
      $scope.todos.sumsalemount = [];
      $scope.todos.gross = [];
      //
      angular.forEach($scope.todos.setDateMonth ,function(data){
        $scope.todos.sumsalemount.push(data.sumsalemount);
        $scope.todos.gross.push(data.gross);
      });

      console.log($scope.todos.sumsalemount);
      console.log($scope.todos.gross);
    }
  }]);